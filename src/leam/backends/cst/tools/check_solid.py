"""Validate CST solids.json against description, parameters, and materials."""
...
from leam.backends.cst.paths import prompt_path as cst_prompt_path
...
class CheckSolid:
    """Run lightweight local and LLM-assisted checks over solids.json."""
    ...
    def __init__(
        ...
        """Initialize prompt set and output directory for solids checks."""
        self.llm_caller = LLMCaller(
            default_model=model,
            reasoning_effort=DEFAULT_REASONING_EFFORT,
        )
        self.backend = str(backend or "cst").strip().lower()
        if self.backend not in {"cst", "hfss"}:
            self.backend = "cst"
        self.prompt_file = (
            cst_prompt_path("check_solid_prompt.md")
            if self.backend == "cst"
            else hfss_prompt_path("check_solid_prompt.md")
        )
        self.builtin_materials = BACKEND_BUILTIN_MATERIALS[self.backend]
    ...
    def _run_local_checks(
        ...
        for index, solid in enumerate(solids_list):
            ...
            if not material:
                ...
                material_key not in self.builtin_materials
